# -*- coding: utf-8 -*-
"""直接测试 - 使用原始 edt2.o（不重新编译），只生成 config.lua 和 sl/map.map"""
...
def main():
    print("=" * 60)
    print("测试：原始 edt2.o + sl/map.map + config.lua 启动 10002")
    print("=" * 60)
    ...
    if sl_file.exists():
        ...
        print(f"[INFO] sl/map.map created: {len(decompressed)} bytes")
    else:
        print(f"[WARN] {sl_file} not found")
