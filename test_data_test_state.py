...
def test():
    ...
    if (data_test / "core" / "sl" / "map.map").exists():
        ...
        print(f"[TEST] 已恢复 core/sl/map.map")

    # 2. 使用 data_test 的 config.lua
    if (data_test / "core" / "config.lua").exists():
        ...
        print(f"[TEST] 已恢复 config.lua")

    # 3. 使用 data_test 的 edt2.lua（文本版）
    # 先删除我们的 edt2.o
    edt2_o = game_dir / "core" / "edt2.o"
    if edt2_o.exists():
        edt2_o.unlink()
        print("[TEST] 已删除 core/edt2.o")
    if (data_test / "core" / "edt2.lua").exists():
        shutil.copy2(data_test / "core" / "edt2.lua", game_dir / "core" / "edt2.lua")
        print("[TEST] 已恢复 core/edt2.lua")

    # 4. 使用 data_test 的 GameSetting.inf
    if (data_test / "core" / "GameSetting.inf").exists():
        ...
        print("[TEST] 已恢复 GameSetting.inf")

    # 5. 删除我们的 map.o（如果有）
    map_o = game_dir / "map.o"
    if map_o.exists():
        map_o.unlink()
        print("[TEST] 已删除 data/map.o")
