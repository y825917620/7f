# -*- coding: utf-8 -*-
"""测试：使用 data_test 根目录的 config.lua 启动游戏."""
...
def test():
    ...
    # 使用 data_test 根目录的 config.lua（非 -1 版本）
    root_config = data_test / "config.lua"
    if root_config.exists():
        shutil.copy2(root_config, game_dir / "config.lua")
        shutil.copy2(root_config, game_dir / "core" / "config.lua")
        print(f"[TEST] 已复制根目录 config.lua: {root_config.read_text()}")

    # 使用 data_test 的 edt2.lua
    edt2_o = game_dir / "core" / "edt2.o"
    if edt2_o.exists():
        edt2_o.unlink()
    if (data_test / "core" / "edt2.lua").exists():
        shutil.copy2(data_test / "core" / "edt2.lua", game_dir / "core" / "edt2.lua")
        print(f"[TEST] 已复制 edt2.lua: {(game_dir / 'core' / 'edt2.lua').read_text()}")
