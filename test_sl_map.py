# -*- coding: utf-8 -*-
"""测试：创建 sl/map.map 从 10002_dec.bin，验证游戏是否能加载地图."""
...
def test():
    ...
    # 关键：创建 sl/map.map 从解码后的地图文件
    dec_bin = game_dir / "map" / "10002_dec.bin"
    if dec_bin.exists():
        # 创建 sl/ 目录和 map.map 文件
        sl_dir = game_dir / "sl"
        sl_dir.mkdir(exist_ok=True)
        sl_map = sl_dir / "map.map"
        shutil.copy2(dec_bin, sl_map)
        print(f"[TEST] 已创建 {sl_map} ({sl_map.stat().st_size} bytes)")

        # 同时创建 core/sl/map.map
        core_sl_dir = game_dir / "core" / "sl"
        core_sl_dir.mkdir(exist_ok=True)
        core_sl_map = core_sl_dir / "map.map"
        shutil.copy2(dec_bin, core_sl_map)
        print(f"[TEST] 已创建 {core_sl_map} ({core_sl_map.stat().st_size} bytes)")
    else:
        print(f"[TEST] 警告: 未找到 {dec_bin}")
    ...
    if log_dirs:
        ...
        if error_log.exists():
            ...
            if "tab_interface" in content:
                ...
                return False
            elif "读取地图" in content or "sanguo" in content:
                print("[TEST] 结果: 地图加载相关错误（有进展!）")
                return False
            elif content.strip():
                print("[TEST] 结果: 有其他错误")
                return False
